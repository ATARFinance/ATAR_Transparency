...
contract SousChef {
    ...
    // The SYRUP TOKEN!
    IBEP20 public dilithium;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Dilithium tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = dilithium.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 dilithiumSupply = dilithium.balanceOf(address(this));
        if (dilithiumSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(dilithiumSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Dilithium tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        dilithium.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Dilithium tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        dilithium.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        dilithium.safeTransfer(address(msg.sender), user.amount);
    ...
    function updateEmissionRate(uint256 _rewardPerBlock) public
    {

        require(dilithium.getOwner() == msg.sender);
        updatePool();
